...
SELECT SUM(TRIGIASAUKM) AS DOANHTHU ,DAY(NGHD) AS NGAY FROM HOADON WHERE MONTH(NGHD) = MONTH(GETDATE()) GROUP BY NGHD ORDER BY NGHD ASC 

SELECT TOP 31 SUM(TRIGIASAUKM) AS DOANHTHU ,DAY(NGHD) AS NGAY FROM HOADON GROUP BY NGHD ORDER BY NGHD DESC

SELECT TOP 28 SUM(TRIGIASAUKM) AS DOANHTHU ,DAY(NGHD) AS NGAY FROM HOADON GROUP BY NGHD ORDER BY NGHD DESC

SELECT TENSP,SUM(SOLUONG) FROM CTHD CT, SANPHAM SP,HOADON HD WHERE CT.SOHD = HD.SOHD AND CT.MASP = SP.MASP AND DAY(NGHD) = DAY(GETDATE()) GROUP BY TENSP

SET DATEFORMAT DMY 

DELETE FROM NHANVIEN WHERE MANV = 61

delete from account where matk = 16